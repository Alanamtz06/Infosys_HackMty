...
PROMPT_TEMPLATE = """Eres un asistente que audita decisiones de un repartidor en Monterrey.
Explica en 2-3 oraciones, en espanol y lenguaje natural, por que la siguiente jugada
fue {decision}:

Tarifa: ${fare:.2f} MXN
Distancia: {distance_km:.2f} km
Tiempo estimado: {time_minutes:.1f} min
Costo de gasolina: ${gas_cost:.2f} MXN
Costo de tiempo: ${time_cost:.2f} MXN
Score (ganancia neta estimada): ${score:.2f} MXN
"""
...
def audit_decision(evaluation: OrderEvaluation) -> str:
    ...
    prompt = PROMPT_TEMPLATE.format(
        decision="aceptar" if evaluation.should_accept else "rechazar",
